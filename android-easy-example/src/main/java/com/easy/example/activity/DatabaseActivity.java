...
public class DatabaseActivity extends BaseActivity {
	 ...
	 * @param v
	 */
	@Event(value = R.id.btn_read_all, type = View.OnClickListener.class)
	private void onClickReadAll(View v) {
	 ...
	 * @param v
	 */
	@Event(value = R.id.btn_delete_all, type = View.OnClickListener.class)
	 ...
	 * @param v
	 */
	@Event(value = R.id.btn_read, type = View.OnClickListener.class)
	private void onClickRead(View v) {
		if (EmptyUtil.isEmpty(et_id.getText().toString())) {
			ToastUtil.show(self, "请输入id");
		} else {
			User user = dbHelper.read(Integer.parseInt(et_id.getText().toString()));
			if (user == null) {
				ToastUtil.show(self, "找不到结果");
			} else {
				et_name.setText(user.name);
				rg_gender.check(user.gender == 0 ? R.id.rb_female : R.id.rb_male);
			}
		}
	}
	 ...
	 * @param v
	 */
	@Event(value = R.id.btn_insert, type = View.OnClickListener.class)
	private void onClickInsert(View v) {
		if (EmptyUtil.isEmpty(et_id.getText().toString())) {
			...
			user.gender = rg_gender.getCheckedRadioButtonId() == R.id.rb_female ? 0 : 1;
	 ...
	 * @param v
	 */
	@Event(value = R.id.btn_update, type = View.OnClickListener.class)
		...
		public User read(final int id) {
			return agentRead(new Reader<User>() {
				...
				public User doRead(SQLiteDatabase db) {
					Cursor c = db.query("user", null, "id = " + id, null, null, null, null);
					if (c.moveToNext()) {
						return read(c);
					} else {
						return null;
	...
	class User {
		...
		public String toString() {
			return "User [id=" + id + ", name=" + name + ", gender=" + (gender == 0 ? "female" :
					"male") + ", createTime=" + createTime + "]";
		}
	}
}
