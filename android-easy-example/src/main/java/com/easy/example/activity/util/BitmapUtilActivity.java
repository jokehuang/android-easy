...
@ContentView(R.layout.activity_bitmap_util)
public class BitmapUtilActivity extends BaseActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        Bitmap tempBmp = BitmapUtil.decodeByUri(this, data.getData(), iv_src.getWidth(), iv_src.getHeight(), Bitmap
                .Config.RGB_565);
        if (tempBmp == null) {
        ...
        //获得正向的宽高
        String width = info.get("width");
        String height = info.get("height");
        int outWidth = EmptyUtil.isEmpty(width) ? tempBmp.getWidth() : Integer.parseInt(width);
        int outHeight = EmptyUtil.isEmpty(height) ? tempBmp.getHeight() : Integer.parseInt(height);
        if (degress % 180 != 0) {
            int temp = outWidth;
            outWidth = outHeight;
            outHeight = temp;
    ...
    private void onClickSave(View v) {
        if (!isBmpExsit()) return;
        final String path = BITMAP_DIR_PATH + File.separator + System.currentTimeMillis() +
                BitmapUtil.EXT_JPG;
    ...
    private void setBitmap(Bitmap bmp) {
        ...
        tv_current_size.setText(bmp.getWidth() + " x " + bmp.getHeight() + "（" + FileUtil.formatSize(bmp.getByteCount
                ()) + "）");
    }
